...
CELL_SIZE = 30
# Vitesse réelle du jeu : coups par seconde
GAME_SPEED = 5
# IA activée par défaut ; lancer avec --manuel pour jouer au clavier
AI_MODE = '--manuel' not in sys.argv
# Accélération : --x 10 joue 10 fois plus vite. Le temps affiché reste celui
# d'une partie à vitesse réelle (coups / GAME_SPEED), donc indépendant de --x.
ACCEL = float(sys.argv[sys.argv.index('--x') + 1]) if '--x' in sys.argv else 1.0
MAX_FPS = 60
...
def display_info(surface, font, snake, game_time):
...
def main():
    ...
    pygame.display.set_caption(f"Snake - IA x{ACCEL:g}" if AI_MODE else "Snake Classique - Socle de Base")
    ...
    # Chronomètre en temps de jeu : nombre de coups / GAME_SPEED
    steps = 0
    idle = 0  # coups d'attente depuis la dernière pomme
    
    # Vitesse : fps plafonnés, plusieurs coups par image si l'accélération l'exige
    speed = GAME_SPEED * (ACCEL if AI_MODE else 1)
    fps = min(MAX_FPS, speed)
    move_budget = 0.0
    ...
    while running:
        ...
        if not game_over and not victory:
            # Le serpent se déplace à la vitesse définie (speed coups par seconde)
            move_budget += speed / fps
            while move_budget >= 1 and not game_over:
                move_budget -= 1
                if AI_MODE:
                    snake.set_direction(solver.choose(snake.body, snake.grow_pending, apple.position))
                snake.move()
                steps += 1
                if AI_MODE and solver.mode not in ("POMME", "REMPLIR", "ANTICIPE", "DETOUR"):
                ...
                # Fin d'expérience : un tour complet sur lui-même (autant de coups
                # d'attente que sa longueur) sans trouver de chemin sûr vers la pomme
                if AI_MODE and idle > len(snake.body):
                    blocked = True
                    game_over = True
        ...
        apple.draw(screen)
        if AI_MODE:  # chemin prévu par l'IA
            for cx, cy in solver.plan:
                pygame.draw.circle(screen, (90, 90, 200), (cx * CELL_SIZE + CELL_SIZE // 2, cy * CELL_SIZE + CELL_SIZE // 2 + SCORE_PANEL_HEIGHT), 4)
        snake.draw(screen)
        
        # Affiche le score et le temps
        display_info(screen, font_main, snake, steps / GAME_SPEED)
        ...
        if game_over:
            if victory:
                ...
                display_message(screen, font_game_over, "BLOQUÉ", ORANGE)
