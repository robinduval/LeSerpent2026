...
# --- PILOTAGE ALGORITHMIQUE (100 % code, aucun apprentissage) ---
# Ce fichier est le jeu de base ; seule la direction du serpent est décidée par
# un algorithme à chaque déplacement. Grille, horloge (GAME_SPEED) et score
...
# --- ADAPTATEUR ENTRE LE JEU ET LES SOLVEURS ---

REAL_DIRS = [UP, DOWN, LEFT, RIGHT]     # même ordre que core.UP/DOWN/LEFT/RIGHT
...
class GameView:
    """Photo de l'état du vrai jeu, au format attendu par les solveurs (core.Game)."""
    def __init__(self, snake, apple):
        self.body = deque(core.cid(x, y) for x, y in snake.body)
        self.occ = bytearray(core.NN)
        for c in self.body:
            self.occ[c] = 1
        self.dir = REAL_DIRS.index(snake.direction)
        self.grow = snake.grow_pending
        self.apple = core.cid(*apple.position)
        self.head = self.body[0]
        self.tail = self.body[-1]


def agent_direction(agent, snake, apple):
    return REAL_DIRS[agent.choose(GameView(snake, apple))]
...
def main():
    ...
    agent.reset(GameView(snake, apple))
    pygame.display.set_caption(f"Snake - algo ({STRATEGY})")
    ...
    while running:
        ...
        if not game_over and not victory:
            ...
            if move_counter >= GAME_SPEED // 10: # Déplace le serpent à un rythme constant
                snake.set_direction(agent_direction(agent, snake, apple))
                snake.move()
                move_counter = 0
